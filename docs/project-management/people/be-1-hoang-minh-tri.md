...
- File ownership ban đầu: root .NET files, `src/gateway/`, `src/building-blocks/`, `src/services/identity/`, `src/services/transport/`.
- Không tự sửa root npm workspace hoặc Mobile project nếu chưa đồng bộ với Sỹ/Đạt.
...
4. Chạy restore/build/test từ root.
