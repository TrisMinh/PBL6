...
| IBL-003 | 2026-09-10 | Thu gọn repository thành hai vùng chính `src/` và `docs/`. | Accepted, active | Test/infra chuyển vào `src`; executable contracts, SQL baseline, Word snapshots và QA render chuyển vào `docs`; solution và build config vẫn ở root. |
...
Khi source được tạo, nhóm pin một SDK feature band trong `global.json` để local/CI tái lập được build. Dù SDK được pin là 8.x hay bản mới hơn tương thích, mọi `.csproj` vẫn phải chứa:
