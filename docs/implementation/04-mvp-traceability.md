...
Register này là gate lập trình cho MVP 2.0. Có đúng **56 FR MUST active**. Một slice chỉ đạt Done khi endpoint/event/table/UI/test ở cùng hàng đã được triển khai và evidence được gắn vào requirement ID.
...
| SeatHold/Booking | `FR-BOOK-001`..`009`, `FR-BOOK-011` | `getTripSeats`, `createSeatHold`, `getSeatHold`, `releaseSeatHold`, `createBooking`, `listMyBookings`, `getMyBooking`, `previewCancellation`, `cancelBookingItems`, `getTripManifest` | `TripInventoryReady`, `SeatHoldCreated/Expired`, `BookingCreated/Paid/Cancelled`, `RefundRequested` | Booking: trip inventory, hold, booking/item/passenger, cancellation preview | Customer booking/cancel/history; Operator manifest; `TS-SEAT-HOLD`, `TS-BOOK-*`, `TS-CANCELLATION-REFUND` |
| Payment/Refund | `FR-PAY-001`..`009` | `createPayment`, `listBookingPayments`, `getPayment`, `cancelPayment`, `createRefund`, `getRefund`, `searchAdminPayments`, `searchAdminRefunds`, `handlePaymentWebhook` | `PaymentSucceeded/Failed`, `RefundSucceeded/Failed`, `PaymentCompensationRequested` | Payment: payment/attempt/webhook/refund/reconciliation case | Customer payment result; Finance trace; `TS-PAYMENT`, `TS-CANCELLATION-REFUND`, `TS-ADMIN-TRACE` |
| Ticket/check-in | `FR-TICKET-001`..`006` | `listMyTickets`, `getMyTicket`, `validateTicket`, `checkInTicket` | `TicketIssued/Cancelled/CheckedIn` | Booking: tickets + business audit/outbox | Customer ticket; Driver scan/manual entry; `TS-TICKET-VIEW`, `TS-CHECKIN` |
| Operator/Trip | `FR-OPS-001`..`010` | Organization/Bus/Driver/Route/Trip CRUD trong tag `Operator`, `publishTrip`, `cancelTrip`, `transitionTrip`, `getMyAssignments` | `TripPublished/Updated/StatusChanged/Cancelled` | Transport: organization/bus/seat/driver/route/trip/assignment | Backoffice operations; Driver assignments; `TS-OPS-*`, `TS-TRIP-*` |
...
| Platform Admin | `FR-ADMIN-001`..`002` | Admin IAM + `searchBookingsForSupport`, `searchAdminPayments`, `searchAdminRefunds` | không thêm command sửa lịch sử | Identity/Booking/Payment audit và read model | Admin IAM/transaction trace; `TS-ADMIN-IAM`, `TS-ADMIN-TRACE` |
