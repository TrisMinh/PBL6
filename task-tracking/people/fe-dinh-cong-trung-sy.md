...
- File ownership ban đầu: `workspace/package.json`, `workspace/package-lock.json`, `workspace/tsconfig.base.json`, `workspace/src/apps/customer-web/`, `workspace/src/apps/backoffice-web/`.
...
2. Tạo npm workspace trong `workspace/`, một lockfile, TypeScript strict, Vite, lint và unit test.
3. Chừa `workspace/src/apps/mobile` để Đạt tham gia; không tự scaffold Mobile.
4. Chạy build/test/lint từ `workspace/`, ghi lệnh vào README.
...
| Ngô Thành Đạt | npm workspace, generated client config, validation/session conventions | Mỗi lần đổi package/contract |
